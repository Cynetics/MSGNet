...
def train(args, epoch, loader, dataset, model, optimizer, scheduler, device):
    ...
    loss_list = []
    accuracy_list = []
    for i, (q1, q2, labels, boxes) in enumerate(loader):

        boxes = boxes.to(device); labels = labels.to(device)

        q1 = q1
        q2 = q2
        top = torch.cat([q1,q2], dim=1).to(device)

        B, n_classes_img, _  = labels.shape
        ...
        model.zero_grad()
        target = top
        out, _ = model(top, condition=masks)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_embedding', type=float, default=256)
    parser.add_argument('--channel', type=int, default=128)
    parser.add_argument('--n_res_block', type=int, default=3)
    parser.add_argument('--n_res_channel', type=int, default=128)
    parser.add_argument('--n_out_res_block', type=int, default=0)
    parser.add_argument('--n_cond_res_block', type=int, default=2)
    parser.add_argument('--cond_res_channel', type=int, default=64)
    parser.add_argument('--cond_hidden_dim', type=int, default=128)
