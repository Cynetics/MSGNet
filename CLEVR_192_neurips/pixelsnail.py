...
class CondResNet(nn.Module):
    def __init__(self, vq_num_hiddens, channel, kernel_size, n_res_block, prior_type='bottom'):
        super().__init__()

        self.prior_type = prior_type
        self.reduce_conv = nn.Conv2d(vq_num_hiddens, channel, kernel_size=1, stride=1)
        self.reduce_conv_tanh = nn.Tanh()

        bottom = None
        blocks = [WNConv2d(channel, channel, kernel_size, padding=kernel_size // 2)]
    ...
    def forward(self, input, img=None):
        ...
        out = self.reduce_conv(input)
        out = self.reduce_conv_tanh(out)
...
class PixelSNAIL(nn.Module):
    def __init__(
        ...
        self.input_embedding = nn.Embedding(num_embeddings, num_embeddings)
        self.condition_embedding = nn.Embedding(n_class, cond_hidden_dim)
        ...
        self.horizontal = CausalConv2d(
            num_embeddings, channel, [kernel // 2, kernel], padding='down'
        )
        #nclass
        self.vertical = CausalConv2d(
            num_embeddings, channel, [(kernel + 1) // 2, kernel // 2], padding='downright'
        )
        ...
        if n_cond_res_block > 0:
            if self.prior_type=='top':
                self.cond_resnet = CondResNet(
                    cond_hidden_dim, cond_res_channel, cond_res_kernel, n_cond_res_block, prior_type=self.prior_type
                )
        ...
        out.extend([nn.ELU(inplace=True), WNConv2d(channel, num_embeddings, 1)])
    ...
    def forward(self, input, condition=None, cache=None):
        ...
        #input = (
        #    F.one_hot(input, self.num_embeddings).permute(0, 3, 1, 2).type_as(self.background)
        #)
        # do we really need to relearn this embedding?
        input = self.input_embedding(input).permute(0,3,1,2).type_as(self.background)
        ...
        if condition is not None:
            if 'condition' in cache:
                ...
                condition = self.condition_embedding(condition).permute(0,3,1,2).type_as(self.background)
